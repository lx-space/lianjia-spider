...
def query_community(last_id):
    sql = 'select id, name, location_lat, location_lng, uid ' \
          ...
          'where id > {} ' \
          'and driving_duration_to_swj = 0 ' \
          'order by id ' \
          'limit 100'.format(last_id)
    return db_util.query(sql)


def parse_community_driving(item):
    distance = 0
    duration = 0
    location_lat = item[2]
    location_lng = item[3]
...
def save_community_driving(item, driving):
    sql = 'update community_location ' \
          'set driving_distance_to_swj = {} ,' \
          'driving_duration_to_swj = {} ' \
          'where id = {}'.format(driving[0], driving[1], item[0])
    db_util.execute(sql)
...
def consumer(community_list, consumer_name):
    for item in community_list:
        ...
        driving = parse_community_driving(item)
        save_community_driving(item, driving)
